...
import com.wisedu.scc.love.widget.emoji.EmojiConversionUtil;
import com.wisedu.scc.love.widget.html.span.AdvancedUrlSpan;
...
public class Html {
	...
	private static ImageGetter defImageGetter  = new ImageGetter() {
		
		@Override
		public Drawable getDrawable(String source) {
			Bitmap bitmap = loadBitmap(source);
	...
	public static void setEmSize(int size) {
		// 用EditText的size会使得表情添加上去略小，这边hack一下
		Html.size = size + 10;
	}
	...
	public static Spanned fromHtml(String source) {
		return fromHtml(source, defImageGetter, null);
	}
...
class HtmlToSpannedConverter implements ContentHandler {
	...
	private void handleStartTag(String tag, Attributes attributes) {
		if (tag.equalsIgnoreCase("br")) {
			...
			start(mSpannableStringBuilder, new Em());
	...
	private static void startImg(SpannableStringBuilder text,
		...
		if (d == null) {
			d = Html.getContext().getResources()
	...
	private static void endEm(SpannableStringBuilder text) {
		int len = text.length();
		Object obj = getLast(text, Em.class);
		int where = text.getSpanStart(obj);

		text.removeSpan(obj);
		if (where != len) {
			final String content = text.subSequence(where, len).toString();
			text.setSpan(
					EmojiConversionUtil
							.getInstace(Html.getContext())
							.dealExpression(content, Html.getEmSize()), where, len,
					Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
	}
	...
	private static void endA(SpannableStringBuilder text) {
		...
		if (where != len) {
			...
			if (h.mHref != null) {
				text.setSpan(new AdvancedUrlSpan(h.mHref), where, len,
						Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
			}
		}
	}
